...
test("точное совпадение кода идёт первым — и в своей категории, и среди категорий", () => {
  const project = createProject({ name: "Тест" });
  // «П» — это Переключатель из категории «Выключатели»; «ПК», «ПШ», «ПС» и
  // прочие подсветки начинаются с той же буквы, но точное совпадение кода
  // должно стоять раньше них — и вытащить вперёд свою категорию.
  const first = searchTypes(project, "П")[0];
  assert.equal(first.category.name, "Выключатели");
  assert.equal(first.types[0].code, "П");
...
test("новая категория и длинный код находятся наравне с шаблонными", () => {
  ...
  // В «Свете» вперёд выходит «ПС»: его название совпадает с запросом целиком,
  // а у остальных подсветок запрос — только начало названия.
  assert.deepEqual(codes(searchTypes(withType.project, "подсветка")), [
    ["Шторы", ["ПОДСВЕТКА"]],
    ["Свет", ["ПС", "ПК", "ПШ", "ППл", "ПКШ", "ПЛ", "ПЗ"]],
  ]);
  // «Карниз» есть и в шаблоне («Карниз штор» в своей категории, «Подсветка
  // карниза штор» в свете), и у пользователя: находится и то и другое, и своя
  // категория не теряется.
...
test("«Подсветка» находит саму себя, а не тёзок с приставками", () => {
  ...
  // Подсветок в «Свете» у заказчика шесть; седьмой добавлен руками, чтобы
  // тёзка был не только шаблонный.
  const project = addType(base, { code: "ПОДП", name: "Подсветка потолка", categoryId: light.id }).project;
  const whole = searchTypes(project, "подсветка")[0];
  assert.equal(whole.types[0].code, "ПС");
  assert.deepEqual(
    whole.types.map((type) => type.code),
    ["ПС", "ПК", "ПШ", "ППл", "ПКШ", "ПЛ", "ПЗ", "ПОДП"],
  );
  // Недопечатанное «подсветк» точным ни для кого не стало: порядок — порядок
  // справочника, а «ПС» больше никого не обгоняет.
  const part = searchTypes(project, "подсветк")[0];
  assert.deepEqual(
    part.types.map((type) => type.code),
    ["ПК", "ПС", "ПШ", "ППл", "ПКШ", "ПЛ", "ПЗ", "ПОДП"],
  );
});

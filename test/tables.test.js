...
test("знак типа — одна колонка: у точечного фигура, у линейного начертание со словом «линия»", () => {
  ...
  // вниз, ища знак, и приставка отодвигала бы его вправо на восьми строках из
  // двадцати восьми.
  assert.equal(signOf("В"), "Квадрат (как у категории)");
  assert.equal(signOf("Р"), "Круг с двумя точками (как у категории)");
  assert.equal(signOf("ТР"), "Сплошная, линия (как у категории)");
  // Все восемь унаследованных строк шаблона — и ни одной лишней.
  assert.deepEqual(
...
test("справочник типов: разбивка доезжает в Markdown и в буфер, а в CSV — колонкой", () => {
  ...
  assert.ok(markdown.includes("| ТР | Трек | Сплошная, линия (как у категории) |"));

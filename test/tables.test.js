...
test("справочник типов — легенда листа: код, название, знак", () => {
  ...
  assert.deepEqual(
    ...
    ["Т", "С", "ПК", "ТР", "ПС", "Л", "ПШ", "ППл", "ПКШ", "ЛЮ", "Н", "Бр", "ЛВ", "ПЛ", "ПЗ",
     ...
     "Д", "КШ", "СУШ", "ДЭП", "РП", "КАМ", "РЕС", "ПРО", "ДП", "ДД", "ДО", "ДПр",
     "Щ", "ЩС", "ВР", "КН", "КВ"],
  );
  assert.equal(tableRowCount(table), 47);
});
...
test("знак типа — одна колонка: у точечного фигура, у линейного начертание со словом «линия»", () => {
  ...
  // Все унаследованные строки шаблона — и ни одной лишней. Их четырнадцать:
  ...
  assert.deepEqual(
    ...
    // prettier-ignore
    ["Т", "С", "ПК", "ПС", "Л", "ППл", "ЛЮ", "Бр", "Д", "КШ", "СУШ", "ДЭП", "РЕС", "ВР"],
  );
...
test("справочник типов разбит по категориям, в заголовке — цвет словами и кодом", () => {
  ...
  assert.deepEqual(table.groups.map((group) => group.color), [
    ...
    "#1DAB1D", "#001BDD", "#164E63", "#6E4B1F", "#E80098",
  ]);

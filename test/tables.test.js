...
test("справочник типов — легенда листа: код, название, знак", () => {
  ...
  assert.deepEqual(rows[0].cells, ["Т", "Точечный светильник", "Круг с крестом (как у категории)"]);
  ...
  assert.deepEqual(
    ...
    ["Т", "С", "ПК", "ТР", "ПС", "Л", "ПШ", "ППл", "ПКШ", "ЛЮ", "Н", "Бр", "ЛВ", "ПЛ", "ПЗ",
     "В", "ВВ", "П", "ПП", "ВВВ", "Р", "РC", "Б", "К", "ВЫТ", "ОВ", "W", "ПУ", "ВП", "УК",
     "Д", "КШ", "ВОПРОС", "СУШ", "ДЭП", "РП", "КАМ", "РЕС", "ПРО", "ДП", "ДД", "ДО", "ДПр",
     "Щ", "ЩС", "ВР", "КН", "КВ"],
  );
  assert.equal(tableRowCount(table), 48);
});
...
test("знак типа — одна колонка: у точечного фигура, у линейного начертание со словом «линия»", () => {
  ...
  assert.equal(signOf("ПУ"), "Квадрат с разъёмом");
  assert.equal(signOf("В"), "Квадрат");
  assert.equal(signOf("Р"), "Круг с двумя точками");

  // Линейный: начертание, и в конце «линия» — иначе «Двойная» в колонке
  // «Форма» читалась бы как ещё одна фигура.
  assert.equal(signOf("ТР"), "Двойная, линия");
  assert.equal(signOf("ПШ"), "Пунктирная, линия");
  assert.equal(signOf("ПКШ"), "Волнистая, линия");
  assert.equal(signOf("ПЛ"), "Змейка, линия");

  // Знак, взятый у категории, назван унаследованным, но сам знак идёт первым:
  // слова заказчика — «пусть название будет первым». Колонку читают сверху
  // вниз, ища знак, и приставка отодвигала бы его вправо.
  assert.equal(signOf("Т"), "Круг с крестом (как у категории)");
  assert.equal(signOf("КШ"), "Ромб (как у категории)");
  assert.equal(signOf("Л"), "Сплошная, линия (как у категории)");
  // Все унаследованные строки шаблона — и ни одной лишней. Их пятнадцать:
  // справочник заказчика нарочно держит семь типов света под знаком категории.
  assert.deepEqual(
    rows.filter((row) => row.cells[2].includes("(как у категории)")).map((row) => row.cells[0]),
    // prettier-ignore
    ["Т", "С", "ПК", "ПС", "Л", "ППл", "ЛЮ", "Бр", "Д", "КШ", "ВОПРОС", "СУШ", "ДЭП", "РЕС", "ВР"],
  );

  // Название знака не трогается: регистр остаётся тем же, что в словаре.
  ...
  assert.equal(custom.at(-1).cells[2], "Квадрат с разъёмом (как у категории)");
...
test("справочник типов разбит по категориям, в заголовке — цвет словами и кодом", () => {
  ...
  assert.deepEqual(
    ...
    [
      ...
      "Домофон · пурпурный (#D901C4)",
      "Карнизы · коричневый (#9D4C01)",
      "Не назначено · розовый (#FF007F)",
      "Электроприборы · зелёный (#1DAB1D)",
      "Кинотеатр · синий (#001BDD)",
  ...
  assert.deepEqual(table.groups.map((group) => group.color), [
    "#1F6FEB", "#2DA44E", "#D1242F", "#E36209", "#8250DF", "#D901C4", "#9d4c01",
    "#FF007F", "#1DAB1D", "#001BDD", "#164E63", "#6E4B1F", "#E80098",
  ]);
  assert.deepEqual(table.groups.map((group) => group.level), table.groups.map(() => 1));
...
test("справочник типов: разбивка доезжает в Markdown и в буфер, а в CSV — колонкой", () => {
  ...
  assert.equal(csv[3], "Свет;Т;Точечный светильник;Круг с крестом (как у категории)");
  // Начертание с запятой внутри ячейки CSV не разъезжается на две колонки:
  // разделитель здесь «;», и «Двойная, линия» остаётся одной ячейкой.
  assert.ok(csv.includes("Свет;ТР;Трек;Двойная, линия"));
  ...
  assert.ok(markdown.includes("| Л | Лента | Сплошная, линия (как у категории) |"));

  const tsv = toTsv(table).split("\n");
  assert.deepEqual(tsv.slice(0, 3), [
    "Код\tНазвание\tФорма",
    "Свет · синий (#1F6FEB)",
    "Т\tТочечный светильник\tКруг с крестом (как у категории)",
  ]);
  // Запятая в буфер обмена едет как есть: на колонки там режет табуляция.
  assert.ok(tsv.includes("ТР\tТрек\tДвойная, линия"));
});
...
function tablesEquipmentFixture() {
  const box = tablesFixture();
  // Тип берётся из стартового справочника объекта — того самого, что копируется
  // в новый объект вместе с типами меток. У диммера типа нет намеренно: пустой
  // тип законен, и лист обязан это пережить.
  const relayType = equipmentTypesInOrder(box.project).find((item) => item.name === "Реле 2 канала");
  const relay = addEquipment(box.project, {
    name: "Реле двухканальное",
    vendor: "Aqara",
    code: "RL-2",
    typeId: relayType.id,
  });
...
test("таблица оборудования: строка на размещение, модель — колонкой", () => {
  ...
  assert.deepEqual(rows[0].cells, ["Т1", "Реле двухканальное", "Реле 2 канала", "Спальная Оли", "точка под зеркалом", "С1"]);
  // Тип у модели не заполнен — ячейка пустая, и это не поломка.
...
test("лист оборудования, сужённый помещением, показывает только его строки", () => {
  ...
  assert.deepEqual(table.groups[0].rows.map((row) => row.cells.slice(0, 4)), [["Р1", "Диммер", "", "Холл"]]);
...
test("размещение без модели, без метки и с потерянной связью не выглядит нормальной строкой", () => {
  ...
  assert.equal(lostModel.cells[5], "ссылка потеряна");
});
...
test("Markdown и буфер обмена: модель в строке, заголовков модели нет, подвал на месте", () => {
  ...
  assert.ok(markdown.includes("| Обозначение | Модель | Тип оборудования | Помещение | Расположение | Связанные метки |"));
  assert.ok(markdown.includes("| Т1 | Реле двухканальное | Реле 2 канала | Спальная Оли | точка под зеркалом | С1 |"));
  ...
  assert.deepEqual(tsv, [
    "Обозначение\tМодель\tТип оборудования\tПомещение\tРасположение\tСвязанные метки",
    "Т1\tРеле двухканальное\tРеле 2 канала\tСпальная Оли\tточка под зеркалом\tС1",
    "Т2\tРеле двухканальное\tРеле 2 канала\tСпальная Оли\tнад кроватью\t",
...
test("CSV оборудования — таблица для Excel: те же колонки, закупка за пустой строкой", () => {
  const { box } = tablesEquipmentFixture();

...
// Т2 лежат — приходилось искать глазами. Кнопка зажигает на плане три рода
// соединений сразу: управление (дуга со стрелкой), общее управление (прямая с
// засечками между равными) и общий номер (оболочка вокруг группы).
//
// **Кнопка одна на все три рода.** Три кнопки в шапке — приборная панель, а
// история у них одна: «покажи разбор». Выключать паутину частями есть чем и
// без второй кнопки — выделением (см. `canvas.canvasFrameLinks`).
//
...
export function linksNotice(project, scheme, filter) {
  ...
  const drawn = links.lines.length + links.ties.length + links.groups.length;
  if (drawn === 0 && away === 0) return strings.links.none;

...
// Первая буква со строчной: «Круг с крестом» → «как у категории: круг с
// крестом». Своего `toLowerCase` на всю строку тут быть не должно — он съел
// бы «RJ45» и «(точка-тире)».
function tableLowerFirst(value) {
...
//
// Унаследованный знак назван унаследованным — теми же словами, что и в
// справочнике («Как у категории: круг с крестом»). Без этого правка формы
// категории молча меняла бы восемь строк листа из двадцати восьми, а по
// распечатке было бы не понять, какие именно.
function tableTypeSign(project, type) {
  ...
  if (own) return name;
  return text(line ? "dictionary.inheritLine" : "dictionary.inheritShape", { name: tableLowerFirst(name) });
}
